	...
	for _, status := range testStatuses {
		if status.Status == 777 {
			...
			if status.Status == 2 {
				...
				st = fmt.Sprintf("%d", status.Status)
			}
			html += cook_html_tests(status, st)
		}
	}
...
func cook_html_tests(status TestStatus, st string) string {
	//fmt.Println("cook_html_tests")
	var html string
	conflUrl := "https://confluence.veon.com/pages/viewpage.action?pageId=%d"
	...
	if status.Status == 2 {
		html += fmt.Sprintf("<details class=\"test-details\">"+
			...
			"</tbody>"+
			"</table>"+
			"</summary>", status.TestData.Bucket, t1.Format(time.RFC3339), status.TestData.Type, status.TestData.Application, st)
		html += "<div style=\"text-align: center;\">Параметры теста</div>"
	...
	i := 0
	html += "<tr>"
	//fmt.Println("ConfID = ", status.TestData.ConfID)
	if status.TestData.ConfID == 0 {
		html += fmt.Sprintf("<td>ConfID = %s</td>", "Отчета нет")
	} else if status.TestData.ConfID == 1 {
		html += fmt.Sprintf("<td>ConfID = %s</td>", "Отчет сгенерирован с ошибкой")
	} else {
		html += fmt.Sprintf(`<td><a href="%s" target="_blank">Отчёт = %d</a></td>`, fmt.Sprintf(conflUrl, status.TestData.ConfID), status.TestData.ConfID)
	}
	html += fmt.Sprintf("<td>Delivery = %s</td>", status.TestData.Delivery)
	html += fmt.Sprintf("<td>Gitlab = %s</td>", status.RequestData.Gitlab)
	html += fmt.Sprintf("<td>Count = %d</td>", status.RequestData.Count)
	html += fmt.Sprintf("<td>Resource = %s</td>", status.RequestData.Resource)
	html += "</tr>"
	...
	if status.Status == 2 {
		html += "<button class=\"tableButton b_restart\" disabled>Restart</button>"
	} else {
		html += "<button class=\"tableButton b_restart\" disabled>Restart</button>"
	}

	if status.RequestData.Gitlab == "" {
		html += "<button class=\"tableButton\" disabled>Stop</button>"
	} else {
		html += "<button class=\"tableButton b_stop\" onclick=\"stop('" + status.State + "')\">Stop</button>"
	}
	if status.Status == 2 {
		html += "<button class=\"tableButton b_grafana\" onclick=\"openNewTab('" + status.TestData.Bucket + "','" + status.TestData.Application + "')\">Grafana</button>"
	} else {
		html += "<button class=\"tableButton b_grafana\" disabled>Grafana</button>"
	}
	html += "</div>"
	html += "</details>\n"
	return html
